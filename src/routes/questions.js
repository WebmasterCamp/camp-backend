...
router.get('/', requireMatchedMajor, async (req, res) => {
  try {
    ...
    const users = await User.find(status === 'all' ? {} : { status }).populate('questions');
    ...
    const filterQuestions = filterUsers.map((filterUser) =>
      filterSelectedFields(
        filterUser,
        ['questions', 'status', '_id', 'firstName', 'lastName', 'academicYear', 'no']
      )
    );
...
router.get('/:id', requireRoles(
  ...
  try {
    ...
    if (_.includes(permissions[req.admin.role], user.questions.major)) {
      const filterUser = filterSelectedFields(
        user,
        ['no', 'facebook', 'questions', 'status', 'event', 'portfolio', 'prominentPoint', 'whyJoinYwc', 'academicYear', 'firstName', 'lastName', '_id', 'university']
      );
